 ...
 * @author: zhouchong
 * 创建日期: 2020/9/2 13:28
 * 描述: Activity 基类 使用 ViewBinding 方式 和 layoutRes 两种方式
 * 修改人:
 * 迭代版本:
 * 迭代说明:
 */
abstract class BaseViewBindingIntActivity : AppCompatActivity(), RequestLifecycle {
    /**
     * 基类里面使用
     */
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (getNormalLayoutResId() != 0) {
            setContentView(getNormalLayoutResId())
        } else {
            setContentView(getViewBindingLayoutResId())
        }
        setupViews()
        activity = this
    ...
    /**
     * 返回 layout 布局
     * @return 为 ViewBinding
     */
    open fun getViewBindingLayoutResId(): View? = rootView

    /**
     * 普通的 layout res
     */
    abstract fun getNormalLayoutResId(): Int
