...
/**
 * @author: zhouchong
 * 创建日期: 2020/9/21 14:28
 * 描述: 横向 recyclerview 自动对齐
 * 修改人:
 * 迭代版本:
 * 迭代说明:
 */
class GalleryPagerSnapHelper : SnapHelper() {
    private val INVALID_DISTANCE = 1f
    private var MILLISECONDS_PER_INCH = 40f
    ...
    private fun findStartView(
        ...
        return if (layoutManager is LinearLayoutManager) {
            ...
            if (horizontalHelper.getDecoratedEnd(firstChildView) >= horizontalHelper.getDecoratedMeasurement(
                    firstChildView) / 2 && horizontalHelper.getDecoratedEnd(firstChildView) > 0
            ) {
                firstChildView
            } else {
